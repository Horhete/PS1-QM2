...
while (testr>0.001)
    ...
    k1= q4_aiyagari(r0); %calculating asset supply
...
%%
figure(1)
plot(kap,[kap' kopt(1,:)' kopt(3,:)' kopt(5,:)'],'linewidth',2)
ylabel('next period asset holdings')
xlabel('current asset holdings')
hleg1 = legend('Constant Asset Line','Lowest Skill Shock', 'Intermediate Skill Shock', 'Highest Skill Shock');

figure(2)
plot(kap,[dist(1,:)' dist(5,:)'],'linewidth',2)
ylabel('distribution')
...
figure(3)
plot(s',[dist(:,1) dist(:,nkap)],'linewidth',2)
ylabel('distribution')
xlabel('Skill Shock')
...
figure(4)
plot(kap,[cons(1,:)' cons(3,:)' cons(5,:)'],'linewidth',2)
ylabel('consumption')
xlabel('asset holdings')
hleg1 = legend('Lowest Skill Shock', 'Intermediate Skill Shock', 'Highest Skill Shock');

figure(5)
plot(kap,[v(1,:)' v(3,:)' v(5,:)'],'linewidth',2)
